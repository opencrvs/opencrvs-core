...
tmux send-keys -t opencrvs 'echo "first command in 1st pane"' C-m

#rename pane 0 with value of $pan1
tmux set -p @mytitle "1"

#split window vertically
tmux split-window -h
tmux send-keys -t opencrvs 'echo "first command in 2nd pane"' C-m
tmux set -p @mytitle "2"

tmux split-window -v -f
tmux send-keys -t opencrvs 'echo "first command in 2nd pane"' C-m
tmux set -p @mytitle "3"

tmux split-window -h
tmux send-keys -t opencrvs 'echo "first command in 2nd pane"' C-m
tmux set -p @mytitle "4"
