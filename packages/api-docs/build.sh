...
touch $EVENTS_YML
(NODE_ENV=develop pnpm --dir $WORK_DIR/../events exec tsx src/openapi.ts) > $EVENTS_YML

# Build OpenAPI docs for country config
COUNTRYCONFIG_YML="$WORK_DIR/build/countryconfig/openapi.yml"
touch $COUNTRYCONFIG_YML
(NODE_ENV=develop pnpm --dir $WORK_DIR/../commons exec tsx src/countryconfig/openapi.ts) > $COUNTRYCONFIG_YML
