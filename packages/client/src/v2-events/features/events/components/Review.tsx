...
/**
 * Content does not pad its own body, so the form supplies it. The read-only
 * Record tab does not need it — there the Content header sets the rhythm.
 */
const FormBody = styled.div<{ $padded: boolean }>`
  ${({ $padded }) => $padded && 'padding-top: 24px;'}
  @media (max-width: ${({ theme }) => theme.grid.breakpoints.md}px) {
