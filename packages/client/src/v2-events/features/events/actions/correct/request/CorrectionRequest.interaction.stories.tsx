...
// Known issue where address is shown as changed when opening the form and coming back.
export const AddressStaysUnchangedInSummaryWhenNotEdited: Story = {
  parameters: {
    chromatic: { disableSnapshot: true },
    offline: {
      drafts: [],
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Open form and go back', async () => {
      const changeAll = await canvas.findAllByRole('button', {
        name: 'Change all'
      })

      await userEvent.click(changeAll[0])
