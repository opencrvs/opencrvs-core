...
/*
 * This logic overrides the default behaviour of "api.event.read"
 ...
 */
setQueryDefaults(trpcOptionsProxy.event.read, {
  queryFn: async (...params) => {
    ...
    if (!meta) {
      throw new Error(
        'api.event.read was called without passing mandatory event configuration'
      )
    }

    const queryOptions = trpcOptionsProxy.event.read.queryOptions(input.input)
...
export function useReadEvent() {
  ...
  return {
    useQuery: (id: string, enabled?: boolean) => {
      ...
      const { queryFn, ...options } = trpc.event.read.queryOptions(id)

      return useQuery({
        ...options,
        enabled: enabled || true,
        queryKey: trpc.event.read.queryKey(id),
        meta: {
          eventConfig
        }
      })
...
/**
 * When full history of an event is not needed, this hook can be used to read the current state of an event.
 * @returns convenience hooks for getting EventState from EventDocument.
 */
export function useReadEventState() {
  const readEvent = useReadEvent()

  return {
    useQuery: (id: string) => {
      const response = readEvent.useQuery(id)
    ...
    useSuspenseQuery: (id: string): EventIndex => {
      const [eventDocument] = readEvent.useSuspenseQuery(id)
