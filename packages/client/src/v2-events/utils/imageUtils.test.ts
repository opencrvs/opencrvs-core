...
import {
  ...
  validateImage
} from '@client/v2-events/utils/imageUtils'
...
describe('Test validateCertificateTemplate function is file is not SVG', () => {
  ...
  it('Should return file as text string if file is SVG', async () => {
    await expect(validateCertificateTemplate(file)).rejects.toThrowError(
      'imageType'
    )
  })
})
...
describe('Test validateImage function if file is not JPG/PNG/JPEG', () => {
  ...
  it('Should return error if file is SVG', async () => {
...
describe('Test validateImage function if file is larger than 5Mb', () => {
  ...
  it('Should return error if file greater than 5Mb', async () => {
    await expect(validateImage(file)).rejects.toThrow()
  })
})
