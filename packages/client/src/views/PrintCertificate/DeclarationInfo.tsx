...
type Key = keyof IDeclarationData | keyof IAdditionalInfo | 'brn' | 'drn'
export const GetDeclarationInfo = ({
  ...
  const orderedKeys = [
    ...order,
    ...Object.keys(info).filter((key) => !order.includes(key as Key))
  ]
  ...
  return (
    <>
      <div>
        {orderedKeys.map((key) =>
          renderDataItem(key as string, info[key as string])
        )}
      </div>
