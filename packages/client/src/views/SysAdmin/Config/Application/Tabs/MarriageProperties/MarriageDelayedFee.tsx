...
export function MarriageDelayedFee() {
  const intl = useIntl()
  const dispatch = useDispatch()
  const offlineCountryConfiguration = useSelector((store: IStoreState) =>
    getOfflineData(store)
  )
  const [marriageDelayedFee, setMarriageDelayedFee] = React.useState(
    offlineCountryConfiguration.config.MARRIAGE.FEE.DELAYED.toLocaleString()
  )
  const [showModal, setShowModal] = React.useState(false)
  const toggleModal = () => {
    setShowModal((prev) => !prev)
    setMarriageDelayedFee(
      offlineCountryConfiguration.config.MARRIAGE.FEE.DELAYED.toLocaleString()
    )
  }
  ...
  const handleMarriageDelayedFee = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const value = String(event.target.value)
    setMarriageDelayedFee(getFormattedFee(value))
  }

  async function marriageDelayedFeeMutationHandler() {
    toggleModal()
    try {
      await callApplicationConfigMutation(
        MarriageActionId.MARRIAGE_DELAYED_FEE,
        {
  ...
  const item = {
    ...
    value: (
      <Currency
        value={offlineCountryConfiguration.config.MARRIAGE.FEE.DELAYED}
    ...
    action: {
      id: MarriageActionId.MARRIAGE_DELAYED_FEE,
      label: intl.formatMessage(buttonMessages.change)
    }
  }
  const id = MarriageActionId.MARRIAGE_DELAYED_FEE
    ...
    <>
      ...
      <ResponsiveModal
        ...
        actions={[
          ...
          <ApplyButton
            ...
            id="apply_change"
            disabled={
              !Boolean(marriageDelayedFee) ||
              notificationStatus === NOTIFICATION_STATUS.IN_PROGRESS
            }
            onClick={() => {
              marriageDelayedFeeMutationHandler()
            }}
          >
      ...
      >
        <Content>
          <Field>
            <InputField
              id="applicationMarriageDelayedFee"
              touched={true}
              required={false}
            >
              <InputContainer>
                ...
                <HalfWidthInput
                  ...
                  type="text"
                  error={false}
                  value={marriageDelayedFee}
                  onChange={handleMarriageDelayedFee}
                />
              </InputContainer>
            </InputField>
          </Field>
      ...
      {notificationStatus !== NOTIFICATION_STATUS.IDLE && (
        ...
        >
          {notificationStatus === NOTIFICATION_STATUS.IN_PROGRESS
            ...
            : notificationStatus === NOTIFICATION_STATUS.SUCCESS
            ? intl.formatMessage(
                messages.applicationMarriageDelayedFeeChangeNotification
              )
