...
test('verifyLoggedInUserPassword returns user info when password matches', async () => {
  ...
  const result = await client.user.verifyLoggedInUserPassword({
...
test('verifyLoggedInUserPassword throws UNAUTHORIZED when password is wrong', async () => {
  const { user } = await setupTestCase()
  ...
  await expect(
    client.user.verifyLoggedInUserPassword({
