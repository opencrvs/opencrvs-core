...
test('Check scopes against event.actions.unarchive', async () => {
  await createIndex(
  ...
  // Records are seeded already archived (DECLARE, then ARCHIVE) so UNARCHIVE is a
  // valid action against them, then UNASSIGN so the fixture matches the pattern the
  // property test relies on (each attempt re-assigns the record to the acting user).
  const { users, isUnderAdministrativeArea, eventIds } =
    await setupScopeTestFixture(573122, [
      ActionType.DECLARE,
      ActionType.ARCHIVE,
      ActionType.UNASSIGN
    ])
