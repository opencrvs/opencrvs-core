...
export function getInvalidUpdateKeys<T>({
  ...
  return (
    updateEntries
      // Fields present in update but removed during cleaning are invalid/disallowed
      // (null values are intentional removals, so they're valid)
      .filter(([key, value]) => !isNull(value) && !cleanedKeys.includes(key))
      .map(([key, value]) => ({
