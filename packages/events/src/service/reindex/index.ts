...
async function reindexBatchToCountryConfig(
  ...
  logger.info(`Batch ${batch[0].id}: Reindex batch to country config took ${new Date().valueOf() - start.valueOf()} ms`)
  if (!response.ok) {
    throw new Error(
...
async function reindexSearch(
  ...
  async function flush() {
    ...
    const batch = buffer
    buffer = []
    logger.info(`Batch ${batch[0].id}: ${batch.length} events to index`)
    ...
    const id = batch[0].id
    logger.info(`Batch ${id}: Processing batch took ${new Date().valueOf() - start.valueOf()} ms`)
  }
...
export async function runReindex(token: TokenWithBearer) {
  ...
  const searchIndexingStream = await reindexSearch(
    ...
    async (batchSize) => {
      ...
      logger.info(`Reindex total records processed: ${processedCount}. Per second: ${Math.round(processedCount / ((new Date().valueOf() - start.valueOf()) / 1000))}`)
    }
  )
