...
test('verifyPassHandler should return 200 and the user claims when the user exists and the password hash matches', async () => {
  const spy = jest.spyOn(User, 'findOne').mockResolvedValueOnce({
    mobile: '27555555555',
    passwordHash:
  ...
  expect(res.result).toMatchObject({ claims: ['test'] })
  expect(spy).toBeCalled()
})
