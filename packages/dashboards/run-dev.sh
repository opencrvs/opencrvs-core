...
export OPENCRVS_METABASE_ADMIN_EMAIL=${OPENCRVS_METABASE_ADMIN_EMAIL:-"superuser@opencrvs.org"}
SCRIPT_PATH=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
BUILD_PATH="${SCRIPT_PATH}/build/dist"
export OPENCRVS_METABASE_ADMIN_PASSWORD_SALT=$(uuidgen)
export OPENCRVS_METABASE_ADMIN_PASSWORD=${OPENCRVS_METABASE_ADMIN_PASSWORD:-"password"}
export OPENCRVS_METABASE_ADMIN_PASSWORD_HASH=$(node "${BUILD_PATH}/generatePasswordHash.js" "$OPENCRVS_METABASE_ADMIN_PASSWORD_SALT$OPENCRVS_METABASE_ADMIN_PASSWORD")
