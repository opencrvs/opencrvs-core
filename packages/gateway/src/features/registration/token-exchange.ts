...
export async function tokenExchangeHandler(
  ...
  try {
    if (!recordId)
      throw new Error(
        'Error occured when calling the token exchange handler. Record ID not found!'
      )
    if (!token)
      throw new Error(
        'Error occured when calling the token exchange handler. Token not found!'
      )
    ...
    throw new Error(`Token exchange execution failed!. ${error}`)
  }
}