...
describe('reinstate record endpoint', () => {
  ...
  it('returns OK after archiving and reinstating a birth declaration', async () => {
    ...
    const archivedRecord: ValidRecord = JSON.parse(
      archiveEndpointResponse.payload
    )

    const archivedRecordEntries = archivedRecord.entry
    const archivedRecordEntriesWithTaskHistory = [
      ...archivedRecordEntries,
      ...mockTaskHistoryEntries
    ]
