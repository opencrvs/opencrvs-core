...
describe('markEventAsWaitingValidationHandler', () => {
  ...
  const bundleWithInputOutputDeath: any = cloneDeep(
    testFhirBundleWithIdsForDeath
  )

  bundleWithInputOutput.entry[1].resource.input = input
  bundleWithInputOutputDeath.entry[1].resource.input = input
