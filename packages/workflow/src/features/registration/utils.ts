...
async function getInProgressDeclarationNotificationPayload(fhirBundle: Bundle) {
  const trackingId = getTrackingId(fhirBundle)
  const crvsOffice = await getCRVSOfficeName(fhirBundle)
  const informantName = await getInformantName(
    fhirBundle,
    INFORMANT_SECTION_CODE
  )
  return {
...
async function getCompleteDeclarationNotificationPayload(fhirBundle: Bundle) {
  const trackingId = getTrackingId(fhirBundle)
  const crvsOffice = await getCRVSOfficeName(fhirBundle)
  const informantName = await getInformantName(
    fhirBundle,
  ...
  const name = await getSubjectName(fhirBundle, DECEASED_SECTION_CODE)
  return {
    trackingId,
    crvsOffice,
    name,
...
export async function sendCreateRecordNotification(
  fhirBundle: Bundle,
  ...
  const notificationParams = [
    {
      type: InformantNotificationName.birthInProgressSMS,
      eventType: EVENT_TYPE.BIRTH,
      complete: false,
      getPayload: getInProgressDeclarationNotificationPayload
    },
    {
      type: InformantNotificationName.birthDeclarationSMS,
      eventType: EVENT_TYPE.BIRTH,
      complete: true,
      getPayload: getCompleteDeclarationNotificationPayload
    },
    {
      type: InformantNotificationName.deathInProgressSMS,
      eventType: EVENT_TYPE.DEATH,
      complete: false,
      getPayload: getInProgressDeclarationNotificationPayload
    },
    {
      type: InformantNotificationName.deathDeclarationSMS,
      eventType: EVENT_TYPE.DEATH,
      complete: true,
      getPayload: getCompleteDeclarationNotificationPayload
    }
  ]
  const isComplete = !isInProgressDeclaration(fhirBundle)
  const selectedNotificationParam = notificationParams.find(
    (param) => param.eventType === eventType && param.complete === isComplete
  )
  if (selectedNotificationParam) {
    const informantSMSNotifications = await getInformantSMSNotification(
      authHeader.Authorization
    )
    if (
      !isInformantSMSNotificationEnabled(
        informantSMSNotifications,
        selectedNotificationParam.type
      )
    )
      return
    const payload = await selectedNotificationParam.getPayload(fhirBundle)
    await sendNotification(
      selectedNotificationParam.type,
      recipient,
      authHeader,
