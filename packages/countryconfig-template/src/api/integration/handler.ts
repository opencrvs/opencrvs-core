...
interface IntegrationConfig {
   ...
   * integration with exactly these values so the integrating system can
   * authenticate immediately using the same client id/secret it carries in its
   * own env — no manual "Refresh secret" step required. When omitted, events
 ...
 * already holds — read them from your own env rather than hard-coding them.
 * See `IntegrationConfig` above for what happens when they are omitted.
 *
 * The list ships empty, so an unmodified country configuration registers
 * nothing and this endpoint is a no-op.
 *
 * Example (MOSIP):
 * {
 *   name: 'MOSIP',
...
export async function systemReadyHandler(
  ...
  // Nothing to register means events never has to be reachable for this
  // endpoint to succeed — which is the common case for an unmodified
  // configuration, so it must not depend on EVENTS_URL being correct.
