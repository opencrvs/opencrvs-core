 ...
 * vars are set as expected); otherwise the encouraging opt-in notice. Telemetry
 * is on by default for deployed Testland, so a disabled instance is warned.
 */
export function logTelemetryStartupStatus() {
  if (!env.TELEMETRY_ENABLED) {
    logger.warn(TELEMETRY_DISABLED_NOTICE)
    return
  }
