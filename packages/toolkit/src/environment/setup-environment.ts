  ...
  return ALL_ANSWERS.flatMap((answerObject) => {
    ...
    return questionsThatAreSecretsOrVariables.map(([key, value]) => {
      ...
      if (
        ...
        (existingQuestion.valueType !== 'SECRET' &&
          existingQuestion.valueType !== 'VARIABLE')
      ) {
        throw new Error(`No secret or variable question found for ${key}`)
      }

      const { valueLabel, valueType, scope } = existingQuestion
  ...
  const processedQuestions = questions.flatMap((question) => {
    ...
    if (questionWithVariableLabel.valueType === 'VARIABLE') {
      ...
      if (existingVariable) {
        ...
        if (
          questionWithVariableLabel.type === 'select' &&
          Array.isArray(questionWithVariableLabel.choices)
        ) {
          initial_value = getIndexFromChoices(
            questionWithVariableLabel.choices,
...
;(async () => {
  ...
  const existingEnvironments = await getRepositoryEnvironments(
    octokit,
    githubOrganisation,
    githubRepository
  )
  ...
  const choices = [
    ...
    ...existingEnvironments
      .filter((env) => !defaultChoices.some((choice) => choice.value === env))
      .map((env) => ({
        name: env,
        value: env
      }))
  ...
  if (['development', 'qa'].includes(environment)) {
    ...
    environment_type = await select({
      ...
      choices: [
        {
          name: 'Development/Quality assurance/Testing (no PII data)',
          value: 'non-production'
        },
        {
          name: 'Staging/Production (hosts PII data, requires frequent backups)',
          value: 'production'
        }
      ]
    })
  ...
  if (traefikConfOption === 'static_ssl') {
    ssl_answers = await askQuestionWithEditor(
      staticSSLCertQuestions,
      existingEnvironmentSecrets
    )
  }
  ...
  if (enableEncryption && !encryption_key_defined && !environment_exists) {
    ...
    if (!enableEncryption && !encryption_key_defined && environment_exists) {
      log(
        ...
        kleur
          .bold()
          .grey(
            `Environment ${environment} is already configured, skipping disk encryption question`
          )
      )
    }
    ...
    if (!DISK_SPACE) {
      log(
        kleur.bold().green('✔'),
        kleur.bold().grey('All available disk space at /data will be used')
      )
    } else {
      log(
        kleur.bold().green('✔'),
        kleur
          .bold()
          .yellow('Variable DISK_SPACE is read-only, allocated space:'),
        DISK_SPACE?.value
      )
    }
  }
  ...
  let backupHost =
    findExistingValue('BACKUP_HOST', 'VARIABLE', 'ENVIRONMENT', existingValues)
      ?.value || ''
  let backupType = ''
  ...
  let restoreType =
    ...
    process.env.RESTORE_ENVIRONMENT_MODE ||
    'dump'
  ...
  if (configureBackup) {
    ...
    const backupHostPrivateKeyExists = findExistingValue(
    ...
    if (!backupHostPrivateKeyExists) {
      ...
      log(
        ...
        kleur
          .bold()
          .yellow(`Generated new SSH key pair for backup host: ${backupHost}`)
      )
    }
  } else {
  ...
  if (configureRestore) {
    const env_list_filtered = existingEnvironments.filter(
      (env) => env !== environment
    )
