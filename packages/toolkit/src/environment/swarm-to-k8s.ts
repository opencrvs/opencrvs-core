...
;(async () => {
  ...
  if (['backup', 'jumpbox'].includes(environment)) {
    info(
      `  > ${environment} environment will not be migrated, see migration notes`
    )
    process.exit(0)
  }
  ...
  if (environment === 'staging') {
    ...
    try {
      ...
      error(
        `Failed to create variable RESTORE_ENVIRONMENT_NAME for environment ${environment}:`,
        err
      )
    }
  }
  if (environment === 'production') {
    ...
    for (const [secretName, secretValue] of Object.entries(secretsToCreate)) {
      try {
        ...
        error(
          `Failed to create secret ${secretName} for environment ${environment}:`,
          err
        )
      }
    }
    for (const [variableName, variableValue] of Object.entries(
      ...
      try {
        ...
        log(
          `  ✓ Created variable ${variableName} for environment ${environment}`
        )
      } catch (err) {
        error(
          `Failed to create variable ${variableName} for environment ${environment}:`,
          err
        )
      }
    }
