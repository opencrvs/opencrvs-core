 ...
 */
async function askToAddCurrentUser(
  existingUsers: User[]
): Promise<User | null> {
...
async function addNewUser(existingUsers: User[]): Promise<User | null> {
  ...
  if (ssh_keys.length === 0) {
    warn(
      '⚠️  Warning: No SSH keys provided. User will not be able to login remotely.'
    )
  }
...
async function editUser(user: User): Promise<User> {
  ...
  const displayUserState = () => {
    ...
    console.log('─'.repeat(70))
    console.log(
      `Current Role:      ${updatedUser.role === 'admin' ? '👑 admin' : '👤 operator'}`
    )
    console.log(
      `Current State:     ${updatedUser.state === 'present' ? '✅ present' : '❌ absent'}`
    )
      ...
      case 'keys':
        ...
        if (newKeys.length === 0) {
          ...
          const confirmRemoveKeys = await confirm({
            message:
              '⚠️  Warning: All SSH keys will be removed. Continue with no SSH keys?',
            default: false
          })
...
export async function manageUsers(inventoryPath: string): Promise<User[]> {
  let users: User[] = fs.existsSync(inventoryPath)
    ? parseInventoryFile(inventoryPath)
    : []
