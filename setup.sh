   ...
   :
    if which $i >/dev/null; then
        ...
        if [ $i == "node" ] ; then
            ...
            echo "We recommend you install Node 16.13.1 as this release has been tested on those versions."
...
versionTest=$(do_version_check $myNodeVersion 16.13.1)
if [ "$versionTest" == "LOWER" ] ; then
  echo "Sorry your Node version is not supported.  You must upgrade Node to use a supported version."
  echo "We recommend you install Node v16.13.1 as this release has been tested on that version."
