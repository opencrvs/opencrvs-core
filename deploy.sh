...
docker_stack_deploy() {
  ...
  ssh $SSH_USER@$SSH_HOST 'cd /opt/opencrvs && \ '$environment_variables' \ docker stack deploy --prune -c docker-compose.deps.yml -c docker-compose.yml -c docker-compose.deploy.yml '$environment_compose' '$replicas_compose' --with-registry-auth opencrvs'
}
...
if [[ "$ENV" = "demo" ]]; then
  if [ -z "$TOKENSEEDER_MOSIP_AUTH__PARTNER_APIKEY" ] ; then
      echo 'Error: Missing environment variable TOKENSEEDER_MOSIP_AUTH__PARTNER_APIKEY.'
      print_usage_and_exit
  fi

  if [ -z "$TOKENSEEDER_MOSIP_AUTH__PARTNER_MISP_LK" ] ; then
      echo 'Error: Missing environment variable TOKENSEEDER_MOSIP_AUTH__PARTNER_MISP_LK.'
      print_usage_and_exit
  fi

  if [ -z "$TOKENSEEDER_CRYPTO_SIGNATURE__SIGN_P12_FILE_PASSWORD" ] ; then
      echo 'Error: Missing environment variable TOKENSEEDER_CRYPTO_SIGNATURE__SIGN_P12_FILE_PASSWORD.'
      print_usage_and_exit
  fi
  ENVIRONMENT_VARIABLES="HOSTNAME='$HOST' \
    VERSION='$VERSION' \
...
docker_stack_deploy "$ENVIRONMENT_COMPOSE" "$REPLICAS_COMPOSE" "$ENVIRONMENT_VARIABLES"
