...
DEV_MAP_NAME=cryptfs              # -n, --name
                                  # -pf, --passphraseFilepath (required - path to a file containing the decryption passphrase in the format db.decrypt.passphrase=XXXX. We recommend this file is stored on a USB and removable in emergencies)
# options
...
echo "test passhrase:"
echo $db.decrypt.passphrase

# open the LUKS device and set a mapping name
echo $db.decrypt.passphrase | cryptsetup -d - luksOpen $LOOP_DEVICE $DEV_MAP_NAME || true
