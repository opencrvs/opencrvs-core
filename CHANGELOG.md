...
- Show the back and forward buttons in the app header on desktop. The history navigator was wired into the mobile search header only, so the desktop app bar had no in-app history navigation at all. Back and forward now appear on the left of the workqueue, search and record headers on desktop, and remain hidden on mobile (the mobile search header keeps its existing single back arrow) [#11776](https://github.com/opencrvs/opencrvs-core/issues/11776)
