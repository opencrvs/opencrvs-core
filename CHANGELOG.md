...
- **countryconfig** — the new `/trigger/telemetry` handler decides whether telemetry is enabled (`TELEMETRY_ENABLED`), stamps the instance identity (`country_code`, `organisation`, `domain`, `environment`, `application_name`), and forwards the enriched report through the toolkit's `sendTelemetry` from `@opencrvs/toolkit/telemetry`. Configured via env vars on the countryconfig deployment: `TELEMETRY_ENABLED`, `COUNTRY_CODE`, `ENVIRONMENT_NAME` (reported environment), `ORGANISATION` (empty by default), and the existing `DOMAIN`. Enabled by default in the Helm chart.
- **toolkit** — a new `@opencrvs/toolkit/telemetry` module exposes `sendTelemetry(report)`, which owns the status service URL and the payload `schema_version`. Keeping these in the toolkit means a core release can change the endpoint or the report shape, and country configurations pick the change up on upgrade — an incompatible report now surfaces as a TypeScript error rather than a silent runtime mismatch. Reports are only ever sent from production instances (`NODE_ENV=production`); other environments build the report but skip the send, so staging/QA/local instances cannot reach the status service. When `TELEMETRY_ENABLED` is `false`, countryconfig logs a notice at startup explaining that only anonymous, aggregate metrics would be shared and how to opt in. The `create-countryconfig` scaffolder asks whether to enable telemetry (default yes) and sets the `TELEMETRY_ENABLED` default accordingly. `opencrvs upgrade` brings the whole telemetry setup to a v2.0 country config — it writes the `/trigger/telemetry` handler, registers its route and startup notice in `src/index.ts`, and adds the telemetry env vars (`TELEMETRY_ENABLED`, `COUNTRY_CODE`, `ENVIRONMENT_NAME`, `ORGANISATION`) to `src/environment.ts`. It asks for the instance's ISO alpha-3 country code and organisation; when both are supplied telemetry is enabled with them baked into the env defaults, and if either is left blank (or the upgrade runs non-interactively) telemetry is left disabled. Anything already present is skipped.
