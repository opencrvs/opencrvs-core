...
- Make the Deployment rollout strategy configurable, and default it to `Recreate`, for OpenCRVS services and the MOSIP mock services. Previously every Deployment in `charts/opencrvs-services` and `charts/opencrvs-mosip` used Kubernetes' default `RollingUpdate` (25% max surge/unavailable), which could run old and new pods side by side during a rollout. `Recreate` terminates all existing pods before any replacement pods are created; set `strategy.type: RollingUpdate` globally, or `<service>.strategy.type` per service, to opt back into rolling updates (optionally tuned with `maxSurge`/`maxUnavailable` on `charts/opencrvs-services`).
