...
- Toolkit commands to set up Kubernetes access on a developer machine: `opencrvs environment add-k8s-context <kubeconfig-file>` adds a kubeconfig copied from a server to `~/.kube/config` (with a backup, and without overwriting existing entries), and `opencrvs environment configure-shell` installs the Kubernetes aliases available on the servers for bash and zsh [#13323](https://github.com/opencrvs/opencrvs-core/issues/13323)
