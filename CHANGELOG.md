...
- Return a conflict naming the offending field, instead of an internal server error, when a write trips a unique constraint on a user's email, mobile or username. The application-level duplicate check is broader than the constraints, so this is reachable only when two creates race — but the cause was masked in production and reached the caller as `Internal server error`. [#11207](https://github.com/opencrvs/opencrvs-core/issues/11207)
