...
- Corrected the default country list, which had not been reviewed since 2017. Country names now follow the current [UN M49](https://unstats.un.org/unsd/methodology/m49/overview/) standard forms, and two entries no longer render surrounding quote characters. One country was also declared with another's translation key, which left the other impossible to select; both now have their own key. Implementations maintaining their own `client.csv` need to add the missing `countries.LAO` row — `yarn extract:translations` reports it. The country dropdown is also sorted on the translated label instead of the hardcoded English order, so it stays alphabetical in every language. [#11954](https://github.com/opencrvs/opencrvs-core/issues/11954)
