...
- **toolkit** — a new `@opencrvs/toolkit/telemetry` module exposes `sendTelemetry(report)`, which owns the status service URL and the payload `schema_version`. Keeping these in the toolkit means a core release can change the endpoint or the report shape, and country configurations pick the change up on upgrade — an incompatible report now surfaces as a TypeScript error rather than a silent runtime mismatch. Reports are only ever sent from production instances (`NODE_ENV=production`); other environments build the report but skip the send, so staging/QA/local instances cannot reach the status service. When `TELEMETRY_ENABLED` is `false`, countryconfig logs a notice at startup explaining that only anonymous, aggregate metrics would be shared and how to opt in. Both the `create-countryconfig` scaffolder and `opencrvs upgrade` ask whether to enable telemetry (default yes) and set the `TELEMETRY_ENABLED` default in the country config's `src/environment.ts` accordingly.
