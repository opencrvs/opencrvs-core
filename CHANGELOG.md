...
- Removed the `--docker-swarm` flag from `opencrvs upgrade`. It did nothing: it was accepted, documented as merging `infrastructure/` — which is what it did during the v2.0 upgrade — and then ignored, because `runUpgrade` never read it. Passing it is now an error rather than a silent no-op.
